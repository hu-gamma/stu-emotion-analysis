...
def evaluate(model_path, test_csv, name="V6"):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n{'='*60}")
    print(f"评估: {name} (8类, 去掉中性)")
    print(f"{'='*60}")
    ...
    print(classification_report(y_true, y_pred, labels=list(range(8)),
          target_names=[get_label_name(i) for i in range(8)], digits=4, zero_division=0))

    return {'accuracy': acc, 'macro_f1': macro_f1, 'weighted_f1': weighted_f1}


if __name__ == "__main__":
    evaluate('/mnt/checkpoints/roberta_bilstm_v6/best_model.pt',
             '/mnt/datasets/csv/split_test.csv', name="V6 (8类)")
