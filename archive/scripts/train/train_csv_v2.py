...
训练脚本 V2
改进点:
- 多数据集合并训练 (OCEMOTION + dreaddit_train + ECSA)
- Focal Loss + 类别权重
- WeightedRandomSampler 动态重采样
- Macro F1 作为早停指标

测试评估请使用 test.py 独立脚本
"""
import os
import sys
...
def train(
    ...
    save_dir: str = '/mnt/checkpoints/roberta_bilstm_full',
    ...
    for epoch in range(epochs):
        ...
        val_loss, val_acc, val_macro_f1, _ = evaluate(model, val_loader, criterion, device)
        scheduler.step()
...
def main():
    save_dir = '/mnt/checkpoints/roberta_bilstm_full'
    os.makedirs(save_dir, exist_ok=True)
    ...
    # 加载数据: 合并 OCEMOTION + dreaddit_train + ECSA 作为训练集
    print("\n加载数据集...")
    train_csv_paths = [
        '/mnt/datasets/csv/OCEMOTION_processed.csv',
        '/mnt/datasets/csv/dreaddit_train_processed.csv',
