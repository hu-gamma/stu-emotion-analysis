"""
训练脚本 V3
基于 V2，使用 all_merged_v2_train_only.csv 作为训练数据（含 GoEmotions 翻译数据）
与旧模型在 dreaddit_test 上进行对比
"""
import os
import sys
...
def train(
    ...
    save_dir: str = '/mnt/checkpoints/roberta_bilstm_v3',
    ...
    print(f"\n{'='*60}")
    print(f"开始训练 V3 (含 GoEmotions 数据)")
    print(f"设备: {device}")
    print(f"训练轮数: {epochs}")
...
def main():
    save_dir = '/mnt/checkpoints/roberta_bilstm_v3'
    os.makedirs(save_dir, exist_ok=True)
    ...
    # V3: 使用合并后的 all_merged_v2_train_only.csv 作为训练集
    print("\n加载数据集...")
    train_csv_paths = [
        '/mnt/datasets/csv/all_merged_v2_train_only.csv',
    ]
    test_csv_path = '/mnt/datasets/csv/dreaddit_test_processed_v2.csv'
