"""
V6 模型测试脚本（8类，去掉中性）
与 test.py 保持相同输出格式：report.md / roc_curve.png / confusion_matrix.png
"""
import os
import sys
import argparse
...
from models.model_v6c_multilayer import RoBERTaBiLSTMMultiLayer
...
def save_report_md(result_dir: str, overall: Dict, per_class_lines: List[str],
    ...
    md_content = f"""# V6 情感分析测试报告（8类，去掉中性）
...
def test(
    model_path: str,
    test_csv: str,
    result_base_dir: str = '/mnt/results',
    batch_size: int = 32,
    max_length: int = 128,
    ...
    # 自动检测模型版本
    if 'v7attnres' in model_path.lower() or 'v7_attention_residual' in model_path.lower():
        model = RoBERTaBiLSTMV7AttentionResidual(num_classes=num_classes)
        print("✓ 使用 V7-Attention-Residual 多层融合+注意力池化+残差模型")
    elif 'v7attn' in model_path.lower() or 'v7_attention' in model_path.lower():
        model = RoBERTaBiLSTMV7Attention(num_classes=num_classes)
        print("✓ 使用 V7-Attention 多层融合+注意力池化模型")
    elif 'v7' in model_path.lower():
        model = RoBERTaBiLSTMV7(num_classes=num_classes)
        print("✓ 使用 V7 可配置多层融合模型")
    elif 'v6c' in model_path.lower():
        model = RoBERTaBiLSTMMultiLayer(num_classes=num_classes)
        print("✓ 使用 V6c 多层融合模型")
    elif 'v6b' in model_path.lower():
...
def main():
    parser = argparse.ArgumentParser(description='V6 RoBERTa-BiLSTM 模型测试脚本（8类）')
    parser.add_argument('--model', '-m', type=str,
                        default='/mnt/checkpoints/roberta_bilstm_v6/best_model.pt',
                        help='模型文件路径')
