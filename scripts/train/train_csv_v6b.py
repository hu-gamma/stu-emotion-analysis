"""
训练脚本 V6b
基于 V6a 配置，使用增强训练集（GoEmotions 翻译数据补充厌恶+惊讶）
"""
import os
import sys
...
def train(
    ...
    save_dir: str = '/mnt/checkpoints/roberta_bilstm_v6b',
    ...
    print(f"\n{'='*60}")
    print(f"开始训练 V6b (增强数据: 清洗后 + GoEmotions厌恶/惊讶)")
    print(f"设备: {device}")
    print(f"训练轮数: {epochs}")
...
def main():
    save_dir = '/mnt/checkpoints/roberta_bilstm_v6b'
    os.makedirs(save_dir, exist_ok=True)
    ...
    print("\n加载数据集...")
    train_csv_paths = [
        '/mnt/datasets/csv/split_train_enhanced.csv',
    ]
