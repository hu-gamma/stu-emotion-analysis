"""
训练脚本 V6b-Cleaned
基于 V6a，使用清洗后的 split_train_cleaned.csv + 增强 Dropout 的 RoBERTa-BiLSTM 模型
架构: RoBERTa→Dropout(0.3)→BiLSTM→Dropout(0.3)→Classifier(Dropout0.3)
去掉"中性"类，共8类
"""
import os
import sys
...
def train(
    ...
    save_dir: str = '/mnt/checkpoints/roberta_bilstm_v6b_cleaned',
    ...
    print(f"\n{'='*60}")
    print(f"开始训练 V6b-Cleaned (清洗数据集, 增强Dropout, 去掉中性, 8类)")
    print(f"训练数据: split_train_cleaned.csv")
    print(f"RoBERTa dropout=0.3, LSTM dropout=0.3, Classifier dropout=0.3")
    print(f"设备: {device}")
    print(f"训练轮数: {epochs}")
...
def main():
    save_dir = '/mnt/checkpoints/roberta_bilstm_v6b_cleaned'
    os.makedirs(save_dir, exist_ok=True)
    ...
    print("\n加载数据集...")
    print("使用清洗后的训练集: split_train_cleaned.csv")
    train_csv_paths = [
        '/mnt/datasets/csv/split_train_cleaned.csv',
    ]
    ...
    num_classes = get_num_classes()
    print(f"\n初始化 V6b 模型 (类别数: {num_classes})...")
    model = RoBERTaBiLSTMDropoutEnhanced(
        num_classes=num_classes,
        roberta_dropout_rate=0.3,
