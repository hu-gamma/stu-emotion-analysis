"""
训练脚本 V6c — RoBERTa-BiLSTM + 多层特征融合
融合RoBERTa最后4层transformer输出，通过可学习权重加权聚合
"""
import os
import sys
...
def train(
    ...
    save_dir: str = '/mnt/checkpoints/roberta_bilstm_v6c',
    ...
    print(f"\n{'='*60}")
    print(f"开始训练 V6c (RoBERTa多层融合 + 增强数据, 8类)")
    print(f"设备: {device}")
    print(f"训练轮数: {epochs}")
    print(f"学习率: {learning_rate}")
    print(f"优化器: AdamW")
    print(f"类别数: {num_classes} (多分类)")
    print(f"融合层数: {model.num_fusion_layers}")
    print(f"{'='*60}\n")
    ...
    for epoch in range(epochs):
        ...
        lw_str = ", ".join([f"L{i+1}={w:.3f}" for i, w in enumerate(layer_weights)])
        print(f"  当前层权重: {lw_str}")
...
def main():
    save_dir = '/mnt/checkpoints/roberta_bilstm_v6c'
    os.makedirs(save_dir, exist_ok=True)
    ...
    num_classes = get_num_classes()
    print(f"\n初始化模型 V6c (多层融合, 类别数: {num_classes})...")
    model = RoBERTaBiLSTMMultiLayer(
        num_classes=num_classes,
        dropout_rate=0.1,
    ...
    print(f"初始层权重: {model.get_layer_weights()}")
