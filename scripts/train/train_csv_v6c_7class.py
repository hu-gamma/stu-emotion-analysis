"""
训练脚本 V6c-grid 7类 — RoBERTa-BiLSTM + 可配置多层融合（去掉尴尬类）
支持命令行参数：融合层数、dropout、分层学习率
用法:
    python scripts/train/train_csv_v6c_7class.py \
        --num-fusion-layers 4 \
        --dropout 0.3 \
        --use-layerwise-lr \
        --save-dir checkpoints/roberta_bilstm_v6c_7class_L4_d03
"""
import os
import sys
import argparse
...
def main():
    ...
    if args.save_dir is None:
        ...
        args.save_dir = f"/mnt/checkpoints/roberta_bilstm_v6c_7class_L{args.num_fusion_layers}_d{int(args.dropout*10):02d}_{lw_tag}"
    ...
    config_desc = (f"V6c-grid-7class L{args.num_fusion_layers} d{args.dropout} "
