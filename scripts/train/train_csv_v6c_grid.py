"""
训练脚本 V6c-grid — RoBERTa-BiLSTM + 可配置多层融合
支持命令行参数：融合层数、dropout、分层学习率
用法:
    python scripts/train/train_csv_v6c_grid.py \
        --num-fusion-layers 2 \
        --dropout 0.2 \
        --use-layerwise-lr \
        --save-dir checkpoints/roberta_bilstm_v6c_l2_d02
"""
import os
import sys
import argparse
...
from models.model_v6c_configurable import RoBERTaBiLSTMConfigurable
...
def main():
    ...
    parser.add_argument('--num-fusion-layers', type=int, default=2, choices=[2, 3, 4])
    ...
    if args.save_dir is None:
        ...
        args.save_dir = f"/mnt/checkpoints/roberta_bilstm_v6c_L{args.num_fusion_layers}_d{int(args.dropout*10):02d}_{lw_tag}"
    ...
    if args.use_layerwise_lr:
        ...
        print(f"分层学习率: RoBERTa={args.lr*args.roberta_lr_ratio:.0e}, "
              f"BiLSTM={args.lr*args.bilstm_lr_ratio:.0e}, Classifier={args.lr*args.classifier_lr_ratio:.0e}")
    else:
    ...
    config_desc = (f"V6c-grid L{args.num_fusion_layers} d{args.dropout} "
