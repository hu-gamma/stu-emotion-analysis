"""
训练脚本 V7-Attention 7类 — RoBERTa-BiLSTM + Attention Pooling（去掉尴尬类）
用法:
    python scripts/train/train_csv_v7attn_7class.py \
        --num-fusion-layers 4 \
        --dropout 0.3 \
        --use-layerwise-lr \
        --save-dir checkpoints/roberta_bilstm_v7attn_7class_L4_d03
"""
import os
import sys
import argparse
...
def main():
    ...
    if args.save_dir is None:
        ...
        args.save_dir = f"/mnt/checkpoints/roberta_bilstm_v7attn_7class_L{args.num_fusion_layers}_d{int(args.dropout*10):02d}_{lw_tag}"
    ...
    print(f"模型: V7-Attention (Attention Pooling)")
    print(f"类别数: 7类（去掉尴尬）")
    ...
    model = RoBERTaBiLSTMV7Attention(
    ...
    config_desc = f"V7-Attention-7class L{args.num_fusion_layers} d{args.dropout} {'layerwise' if args.use_layerwise_lr else 'unified'}"
